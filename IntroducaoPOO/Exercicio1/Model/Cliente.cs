...
public class Cliente
{
    ...
    private static string FormataTelefone(string number)
    {
        if (number.Length != 10 && number.Length != 11)
            throw new ArgumentException("O número não corresponde ao padrão telefônico.", nameof(number));
        ReadOnlySpan<char> span = stackalloc char[number.Length];
        span = number.AsSpan();
        var offset = 11 - number.Length;
        return $"({span[..2]}) {span[2..(7 - offset)]}-{span[(7 - offset)..]}";
    }
}