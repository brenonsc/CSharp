...
public class PessoaJuridica : Cliente
{
    ...
    //Construtor
    public PessoaJuridica(int id, string nome, string endereco, string email, string telefone, string cnpj) : base(id,
    ...
    {
        if (number.Length != 14)
            throw new ArgumentException("O número não corresponde ao padrão CNPJ.", nameof(number));
        ReadOnlySpan<char> span = stackalloc char[number.Length];
        span = number.AsSpan();
        return $"{span[..2]}.{span[2..5]}.{span[5..8]}/{span[8..12]}-{span[12..]}";
    }
